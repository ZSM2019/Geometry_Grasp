...
{
    // put in .pcd 
    std::string filename("/home/zsm/catkin_ws/src/rospcl/src/222.pcd");
    ...
    {
        if(cloud->points[xxx].z < 0.65+0.015)
        {
            pcl::PointXYZ point;
    ...
    for(int ddddd = 0;ddddd<cloud_grasp->size();ddddd++)
    {
        ...
        std::stringstream ss;
        ss << "handle" << ddddd;
        viewer->addLine<pcl::PointXYZ>(cloud_grasp->points[ddddd],point,0,255,0,ss.str());
